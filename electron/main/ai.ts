...
**"못 읽은 붙임"** 은 글을 열지 못해 이름만 아는 것입니다. 그래도 우리가 채워 낼
서식일 때가 많으므로 **절차에 그 이름을 그대로 적으세요.** 다만 그 안에 무엇이
들었는지는 모르니 **내용을 지어내지 마세요.** ("「…」 서식을 채워 …에 제출한다" 까지만)
...
- 절차 — 담당자가 그대로 따라 할 수 있게 차례대로 번호를 붙여 적습니다.
  **어떤 서식을(붙임 이름 그대로), 어디에, 어떻게** 내는지가 반드시 들어가야 합니다.
  제목을 말만 바꿔 두 번 적는 것은 절차가 아닙니다.
